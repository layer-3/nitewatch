...
// It queries the database for total withdrawn amounts in the current hour and day.
func (c *Checker) Check(token common.Address, amount *big.Int) error {
	l, ok := c.limits[token]
	if !ok {
		return fmt.Errorf("no limits configured for token %s", token.Hex())
	}
	...
	if l.Hourly != nil {
		...
		if newTotal.Cmp(l.Hourly) > 0 {
			return fmt.Errorf("hourly limit exceeded for %s: %s > %s", token.Hex(), newTotal, l.Hourly)
		}
	}
	...
	if l.Daily != nil {
		...
		if newTotal.Cmp(l.Daily) > 0 {
			return fmt.Errorf("daily limit exceeded for %s: %s > %s", token.Hex(), newTotal, l.Daily)
		}
	}
